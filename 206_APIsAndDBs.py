...
import collections
import tweepy
import twitter_info_example # same deal as always...
import json
import sqlite3
...
def get_user_tweets(name):
    if name in CACHE_DICTION:
        ...
        twitter_results=api.user_timeline(id=name,count=20)
        CACHE_DICTION[name]=twitter_results
...
users_info =[]
tuples=cur.execute('SELECT * FROM Users')
for tup in tuples:
...
screen_names = []
strings=cur.execute('SELECT screen_name FROM Users')
for string in strings:
	strr=str(string)
	strc=strr[2:-3]
	screen_names.append(strc)
	print (screen_names)
...
retweets = []
t=cur.execute('SELECT * FROM Tweets WHERE retweets>10')
for tweet in t:
...
favorites = []
strings=cur.execute('SELECT description FROM Users WHERE num_favs>500')
for string in strings:
	strr=str(string)
	strc=strr[2:-3]
	favorites.append(strc)
	print (favorites)
...
joined_data = []
tuples=cur.execute("SELECT Users.screen_name, Tweets.text FROM Users INNER JOIN Tweets on Users.screen_name= Tweets.user_posted")
for tup in tuples:
tupstr=str(tup)
	tupc=strr[2:-3]
	joined_data.append(tupc)
	print (joined_data)
# Make a query using an INNER JOIN to get a list of tuples with 2 
# elements in each tuple: the user screenname and the text of the 
...
joined_data2 = []
tuples2=tuples=cur.execute("SELECT Users.screen_name, Tweets.text FROM Users INNER JOIN Tweets on Users.screen_name= Tweets.user_posted ORDER BY Tweets.retweets")
for tup in tuples2:
tupstr=str(tup)
	tupc=strr[2:-3]
	joined_data.append(tupc)
